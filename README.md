...
Ans: JSX stands for JavaScript XML. It is used in React because it lets us write HTML-like code inside JavaScript. It makes the code more easy to read and helps us create the UI of the website.

2. What is the difference between props and state?

Ans: Props are used to pass data from a parent component to a child component. Props are read-only, so the child component cannot directly change them.
...
Ans: The useState hook is used to store and update data in a React component. When the state changes, React updates the UI.
...
Ans: The useEffect hook is used to perform side effects in a React component. For example, loading data, calling an API, or doing something after the component renders.
...
Ans: Every item in a .map() list needs a unique key so React can identify each item separately. It helps React know which item was changed, added, or removed.
If we don't give a proper key, React can show warnings and it may not update the list properly.

6. What is conditional rendering? Show one place you used it.

Ans: Conditional rendering means showing something in the UI only when a certain condition is true.
...
Ans: I pass data from a parent to a child by using props.
For example:
<Child name="John" />
